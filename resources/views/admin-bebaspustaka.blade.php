...
<div class="flex items-center justify-around pb-4 bg-white dark:bg-gray-800 p-4 sm:ml-64">
    <div class="navbar-nav flex n">
        ...
        <div class="relative">
            <form action="/bebaspustaka/search" class="form-inline" method="post">
            @csrf
    ...
    <form action="/bebaspustaka" method="post">
        @csrf
        <div class="p-4 sm:ml-64 ">
            <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">      
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>   
                            ...
                            @if(isset($DashboardBP) && !$DashboardBP->isEmpty())
                                @foreach ( $DashboardBP as $BebasPustaka )
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="sticky left-0 top-0 z-10 bg-gray-100 px-1 py-3">
                                            {{ ($DashboardBP->currentPage() - 1) * $DashboardBP->perPage() + $loop->index + 1 }}
                                        </td>
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{$BebasPustaka->Mahasiswa->Nama}}
                                        </th>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Mahasiswa->Email}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Mahasiswa->NPM}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Mahasiswa->ProgramStudi}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->petugas->Nama}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Judul_buku}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Pengrang}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Tahun_terbit}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$BebasPustaka->Tanggal_pengajuan}}
                                        </td>
                                        <td class="px-4 py-2">
                                            <a href="{{ asset('storage/'.$BebasPustaka->Surat_cutiundur)}}">
                                            file
                                            </a>
                                        </td>
                                        <td class="px-4 py-2">
                                            <a href="{{ asset('storage/'.$BebasPustaka->Bukti_sumbangan)}}">
                                            file
                                            </a>
                                        </td>
                                        <td class="px-4 py-2">
                                            <a href="{{ asset('storage/'.$BebasPustaka->Bukti_karyailmiah)}}">
                                            file
                                            </a>
                                        </td>
                                    </tr>
                ...
                <div class="flex items-center ml-4">
                    {{ $DashboardBP->links('pagination::tailwind') }}
                </div>              
            </div>
        </div>
    </div>
