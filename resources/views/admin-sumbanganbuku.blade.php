...
<head>
    ...
    <title>Dashboard Sumbangan Buku</title>
</head>
<body>
       ...
       <ul class="space-y-2 font-medium">
            <li>
                ...
                <ul id="dropdown-example" class="hidden py-2 space-y-2">
                    ...
                    <li>
                       <a href="/penyerahankp" class="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Penyerahan KP</a>
                    </li>
                </ul>
            </li>
...
<div class="flex items-center justify-around pb-4 bg-white dark:bg-gray-800 p-4 sm:ml-64">
    <div class="navbar-nav flex n">
        ...
        <div class="relative">
            <form action="/sumbanganb/search" class="form-inline" method="post">
            @csrf
...
<form action="/DashboardSB" method="post">
        @csrf
            ...
            <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">      
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>  
                            @if(isset($DashboardSB) && !$DashboardSB->isEmpty())
                                @foreach ( $DashboardSB as $SumbanganBuku )
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="sticky left-0 top-0 z-10 bg-gray-100 px-1 py-3">
                                            {{ ($DashboardSB->currentPage() - 1) * $DashboardSB->perPage() + $loop->index + 1 }}
                                        </td>
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{$SumbanganBuku->Mahasiswa->Nama}}
                                        </th>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Mahasiswa->Email}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Mahasiswa->NPM}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Mahasiswa->ProgramStudi}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Nomber_wa}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Periode_wisuda}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Judul_buku}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Pengarang}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Tahun_terbitBuku}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Tanggal_pemesananBuku}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->Tanggal_penyerahanBuku}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$SumbanganBuku->petugas->Nama}}
                                        </td>
                                
                                        <td class="px-4 py-2">
                                            <a href="{{ asset('storage/'.$SumbanganBuku->Bukti_pembelian)}}">
                                            file
                                            </a>
                                        </td>
                                    </tr>
...
=======
                    ...
                    {{ $DashboardSB->links('pagination::tailwind') }}
                </div>    
