...
<div class="flex items-center justify-around pb-4 bg-white dark:bg-gray-800 p-4 sm:ml-64">
    <div class="navbar-nav flex n">
        ...
        <div class="relative">
            <form action="/validasita/search" class="form-inline" method="post">
            @csrf
    ...
    <form action="/validasita" method="post">
        @csrf
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-2 bg-gray-100">
                                    Upload TA/Jurnal?/Tesis 
                                </th>
                                <th scope="col" class="px-4 py-2 bg-gray-100">
                                    Karya Ilmiah
                                </th>
                                <th scope="col" class="px-4 py-2 bg-gray-100">
                                    Verifikasi Link Jurnal
                                </th>
                                
                            </tr>
                        </thead>
                        <tbody> 
                            @if(isset($DashboardLTA) && !$DashboardLTA->isEmpty())
                                @foreach ( $DashboardLTA as $ValidasiTA )
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{$ValidasiTA->Mahasiswa->Nama}}
                                        </th>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Mahasiswa->Email}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Mahasiswa->NPM}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Mahasiswa->ProgramStudi}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Judul_penelitian}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Nama_pembimbing}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Tanggal_sidang}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Periode_wisuda}}
                                        </td>                                       
                                        <td class="px-4 py-2">
                                            <a href="{{ asset('storage/'.$ValidasiTA->Bukti_pembelian)}}">
                                            file
                                            </a>
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Karya_ilmiah}}
                                        </td>
                                        <td class="px-4 py-2">
                                            {{$ValidasiTA->Verifikasi_link}}
                                        </td>
